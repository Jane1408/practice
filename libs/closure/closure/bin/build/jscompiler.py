...
def _GetJsCompilerArgs(compiler_jar_path, java_version, source_paths, flags_file,
  ...
  #collect list of compiler flags
  compiler_flags.extend(['--js ' + path for path in source_paths])
  if jvm_flags:
    compiler_flags.extend(jvm_flags)

  #write flags to file for avoiding problem with too big arguments length
  flags_file.write(" ".join(compiler_flags))
  flags_file.close()
...
def Compile(compiler_jar_path, source_paths,
  ...
  args = _GetJsCompilerArgs(
      compiler_jar_path, java_version, source_paths, flags_file, jvm_flags, compiler_flags)
