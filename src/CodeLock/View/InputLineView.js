goog.provide("CodeLock.View.InputLineView");
...
goog.scope(function() {
     ...
     * @constructor
     */
    CodeLock.View.InputLineView = goog.defineClass(null, {
        constructor: function (data) {
            this._data = data;
            this.position = this._data.getPosition();
            this.size = this._data.getSize();
            this._codeLine = goog.dom.createElement(goog.dom.TagName.INPUT);
            ...
            this._codeLine.addEventListener("input", goog.bind(this.checkValue, this));
        },
        checkValue: function()
        {
            var string = this._codeLine.value;
            if(string.length > 5)
            {
                this._codeLine.value =  string.slice(0, 5);
            }
            this._data.setValue(this._codeLine.value);
        },
